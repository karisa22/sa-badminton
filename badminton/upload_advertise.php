...
if (isset($_POST['submit_advertise'])) {
    if (!empty($_FILES["file"]["name"])) {
        ...
        if (in_array($fileType, $allowTypes)) {
            if (move_uploaded_file($_FILES['file']['tmp_name'], $targetFilePath)) {
                $insert = $db->query("INSERT INTO advertise(advertise_name, uploaded_on) VALUES ('".$fileName."', NOW())");
                if ($insert) {
                    $_SESSION['statusMsg'] = "The file <b>" . $fileName . "</b> has been uploaded successfully.";
                    header("location: advertise_admin.php");
                } else {
